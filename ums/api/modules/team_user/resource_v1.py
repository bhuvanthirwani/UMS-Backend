import logging
from .schema_v1 import RoleUserSchema
...
ROLE_USER_ENDPOINT = "/role-user"


class RoleUserResource(CRUDResource):
    ...
    def post(self):
        ...
        print("here")
        filter = []
        filter.append(RoleUserModel.role_id == entry.role_id)
        filter.append(RoleUserModel.user_id == entry.user_id)
        existing_id, is_acitve = check_independent_resource_existence(RoleUserModel, filter)
        print(f"existing_id: {existing_id}, is_active: {is_acitve}")
        if existing_id:
            if is_acitve == 'False':
                return super().do_update(existing_id, update_obj={"is_active": True})
            else:
                return self.response(code=status.HTTP_400_BAD_REQUEST, message=f"user already exist in role")
        try:
