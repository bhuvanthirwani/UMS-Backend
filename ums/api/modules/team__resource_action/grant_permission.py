...
GRANT_ROLE_PERMISSION_ENDPOINT = "/role-permission"


class GrantRolePermissionResource(BaseResource):
    def __init__(self) -> None:
        self.schema = RoleResourceActionSchema()
        self.model = RoleResourceActionModel()

    @exception_handler
    def post(self):
        entry = dict(request.get_json())
        role_id = entry.get('role_id', None)
        ...
        data: dict = {}
        if res_act_id and role_id:
            filter = []
            filter.append(RoleResourceActionModel.res_act_id == res_act_id)
            filter.append(RoleResourceActionModel.role_id == role_id)
            existing_id, is_acitve = check_independent_resource_existence(RoleResourceActionModel, filter)
            print(f"existing_id: {existing_id}, is_active: {is_acitve}")
            if existing_id:
                if is_acitve == 'False':
                    return self.do_update(existing_id, update_obj={"is_active": True})
                else:
                    return self.response(code=status.HTTP_400_BAD_REQUEST, message=f"Requested permission for the role already exists")
            data = RoleResourceActionModel(role_id=role_id, res_act_id=res_act_id)
            print("data", data)
        try:
