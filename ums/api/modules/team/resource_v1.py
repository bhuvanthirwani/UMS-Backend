import logging
from .schema_v1 import RoleSchema
...
class RolesResource(CRUDResource):
    ...
    def post(self):
        ...
        print("here")
        filter = []
        filter.append(RolesModel.name == entry.name)
        existing_id, is_acitve = check_independent_resource_existence(RolesModel, filter)
        print(f"existing_id: {existing_id}, is_active: {is_acitve}")
        if existing_id:
            if is_acitve == 'False':
                return self.response(code=status.HTTP_400_BAD_REQUEST, message=f"Role with {entry.name} already exists, but is not in active state request admin to re-active or create role with a different name")
            else:
                return self.response(code=status.HTTP_400_BAD_REQUEST, message=f"Role with {entry.name} already exists")
        try:
